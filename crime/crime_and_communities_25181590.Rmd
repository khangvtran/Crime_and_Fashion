...
```{r, echo = F}
(mse_df <- data.frame("Linear"= lm_mse,
...
```{r, echo = F}
target_df <- data.frame("Target" = validation_target, "OLS" = lm_preds,
                         "Ridge" = as.numeric(ridge_preds), "Lasso" = as.numeric(lasso_preds),
                         "KNN" = knn_preds, "Tree" = tree_preds, "RandomForest" = rf_preds)
...
```{r}
train_control <- trainControl(method = "none")
rf_final_model <- train(x = train_validation_features, y = train_validation_target,
                  method = "rf", trControl = train_control)
rf_preds <- predict(object = rf_final_model, newdata = test_features)
```
