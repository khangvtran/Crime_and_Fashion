...
pca <- PCA(X.centered ,scale.unit = FALSE, graph = FALSE,ncp = 100)
```
...
X.10 <- data.frame(Z[,1:100]%*%t(V[,1:100]))
```
...
X.image <- matrix(as.numeric(X.10[2,]), ncol=28, nrow=28, byrow = TRUE) 
X.image <- apply(X.image, 2, rev)
...
We start by dividing the dataset into three parts: The Training set (60 pct.), Validation  set (20 pct.) and Test set (20 pct.).
