...
### Estimating test error
Random forest also turned out to be the best multiclass classifier. We will therefore estimate the test error, using the model and the hold outtest set. We train a random forest model on the combined training and validation set, and estiamte the out-of-sample error rate using the test set. We see that the test error for the the models is very low. We also see in the confusion matrices that the types of errors are very evenly distributed. 
```{r test error mul}
...
yhat.test.mul <- predict(rf.mul.test,newdata = Xy.1[test_idx.1,])
confusion.test.mul <- table(obs = Xy.1[test_idx.1,"y"], pred = yhat.test.1)
error_rate.test.mul <- 1 - (sum(diag(confusion.test.1)) / sum(test_idx.1))
```
